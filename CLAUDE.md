...
- **`managers/EcoSystemManager`** — plant growth consumes bone meal from the roof hopper. `setBoneMeal()` must never build a zero-amount `ItemStack`: Paper 26.2+ throws `IllegalArgumentException: amount must be greater than 0` (fixed in 1.10.2). Treat that as a general rule for any `ItemStack` this addon creates.
